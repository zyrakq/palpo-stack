...
- **internal**: Internal network deployment without port forwarding (use with [Homepage Gateway](../homepage/) for external access on primary domain)
...
The internal environment is designed for deployments where Palpo runs in an isolated internal network without direct external access. This configuration is ideal when using a gateway service (like [Homepage Gateway](../homepage/)) to provide external access on the primary domain.

**Use Case:** When you want Matrix usernames tied to your primary domain (e.g., `@user:example.com`) instead of a subdomain (e.g., `@user:matrix.example.com`), deploy Palpo in internal mode and use Homepage Gateway on the primary domain to route Matrix traffic to the internal Palpo instance.
...
**Note:** For external access in internal mode, deploy [Homepage Gateway](../homepage/) on your primary domain to route Matrix Federation and Client-Server API requests to the internal Palpo instance.
